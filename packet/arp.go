...
type ARPHdr struct {
	...
	SHA       MACAddress                // Sender hardware address (sender MAC address)
	...
	THA MACAddress                // Target hardware address (target MAC address)
...
func InitARPRequestPacket(packet *Packet, SHA MACAddress, SPA, TPA IPv4Address) bool {
...
// SHA.
func InitARPReplyPacket(packet *Packet, SHA, THA MACAddress, SPA, TPA IPv4Address) bool {
	if !initARPCommonData(packet) {
		common.LogWarning(common.Debug, "InitARPRequestPacket: failed to fill common data")
		return false
	}
...
// FF:FF:FF:FF:FF:FF (broadcast) and source address is set to SHA.
func InitGARPAnnouncementRequestPacket(packet *Packet, SHA MACAddress, SPA IPv4Address) bool {
	if !initARPCommonData(packet) {
		common.LogWarning(common.Debug, "InitARPRequestPacket: failed to fill common data")
		return false
	}
...
// to SHA.
func InitGARPAnnouncementReplyPacket(packet *Packet, SHA MACAddress, SPA IPv4Address) bool {
	if !initARPCommonData(packet) {
		common.LogWarning(common.Debug, "InitARPRequestPacket: failed to fill common data")
		return false
	}
